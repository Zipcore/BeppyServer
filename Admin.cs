...
namespace BeppyServer {
    public class Admin {
        ...
        public static void KickPlayer(ClientInfo sender, int entityId, string reason = "") {
            KickPlayer(sender, entityId.ToString(), reason);
        }

        public static void KickPlayer(ClientInfo sender, ulong steamId, string reason = "") {
            KickPlayer(sender, steamId.ToString(), reason);
        }

        public static void BanPlayer(ClientInfo sender, int entityId, string reason = "") {
            BanPlayer(sender, entityId.ToString(), reason);
        }

        public static void BanPlayer(ClientInfo sender, ulong steamId, string reason = "") {
            BanPlayer(sender, steamId.ToString(), reason);
        }
    }
}